...
Detection order: **nested → bucketed → flat**. See [Source repositories](/docs/source-repos#expected-layout).
...
For agents, optional `dependsOnSkills` lists skill ids from frontmatter; `GET /artifacts` also resolves `skillDependencies` with names and descriptions for the UI.
