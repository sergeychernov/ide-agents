...
**ide-agents** is a local admin for **IDE agents and skills** from any git repository — **OpenCode**, **Cursor**, **Claude Code**, and **Codex** (configurable in Settings).
...
- Scans skills in **nested**, **flat**, or **bucketed** layouts and optional `agents/*.md` — see [Source repositories](/docs/source-repos)
- Creates symlinks in each enabled tool’s config dir (e.g. `~/.config/opencode`, `~/.cursor/`, `~/.claude/`, `~/.codex/`) and per-project folders
